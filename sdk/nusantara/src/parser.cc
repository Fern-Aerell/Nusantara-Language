...
void Parser::eat(const TokenType& type) {
    if(this->currentToken.getType() == type) {
        ...
        throw std::runtime_error(this->errorInfo.inLine(
            ...
            std::format("Harusnya '{}' tapi malah '{}'.", toString(type), (this->currentToken.getValue().empty()) ? toString(this->currentToken.getType()) : this->currentToken.getValue())
        ));
    }
}
...
bool Parser::matchOr(const std::vector<TokenType>& types) {
    return std::ranges::any_of(types, [this](const TokenType& type) {return this->match(type);});
}

// Fragment
std::unique_ptr<ParserTree> Parser::fragmentTokenTypeGroup(const std::vector<TokenType>& tokenTypes, const ParserRule& rule) {
    for(const TokenType& type : tokenTypes) {
        if(this->currentToken.getType() == type) {
            std::unique_ptr<ParserTree> result = std::make_unique<ParserRuleTree>(rule);
            std::unique_ptr<ParserTree> token = std::make_unique<ParserTokenTree>(this->currentToken);
    ...
    throw std::runtime_error(this->errorInfo.inLine(
        ...
        this->currentToken.getCharIndex(), 
        this->currentToken.getValue(), 
        std::format("'{}' bukan {} yang valid.", toString(this->currentToken.getType()), toString(rule))
    ));
}

// Parse
std::unique_ptr<ParserTree> Parser::parse() {
    return this->parseNusantara();
}
...
std::unique_ptr<ParserTree> Parser::parseNusantara() {
    ...
    while (!match(TokenType::AKHIR_DARI_FILE)) {
        std::unique_ptr<ParserTree> operasiPenugasan = this->parseOperasiPenugasan();
        nusantara->addChild(std::move(operasiPenugasan));
        std::unique_ptr<ParserTree> titikKoma = std::make_unique<ParserTokenTree>(this->currentToken);
...
std::unique_ptr<ParserTree> Parser::parseOperatorPenugasan() {
    ...
    types.push_back(TokenType::SAMA_DENGAN);
...
std::unique_ptr<ParserTree> Parser::parseOperasiPenugasan() {
    std::unique_ptr<ParserTree> operasiPenugasan = std::make_unique<ParserRuleTree>(ParserRule::operasi_penugasan);
    ...
    while (this->matchOr({TokenType::SAMA_DENGAN})) {
        std::unique_ptr<ParserTree> operatorPenugasan = this->parseOperatorPenugasan();
        operasiPenugasan->addChild(std::move(operatorPenugasan));
        std::unique_ptr<ParserTree> right = this->parseOperasiPenugasanPenjumlahan();
...
std::unique_ptr<ParserTree> Parser::parseOperatorPenugasanPenjumlahan() {
    std::vector<TokenType> types;
    types.push_back(TokenType::TAMBAH_SAMA_DENGAN);
    types.push_back(TokenType::KURANG_SAMA_DENGAN);
    return this->fragmentTokenTypeGroup(types, ParserRule::operator_penugasan);
}
...
std::unique_ptr<ParserTree> Parser::parseOperatorPenugasanPerkalian() {
    std::vector<TokenType> types;
    types.push_back(TokenType::KALI_SAMA_DENGAN);
    types.push_back(TokenType::BAGI_SAMA_DENGAN);
    types.push_back(TokenType::SISA_BAGI_SAMA_DENGAN);
    return this->fragmentTokenTypeGroup(types, ParserRule::operator_penugasan);
}
...
std::unique_ptr<ParserTree> Parser::parseOperatorLogika() {
    std::vector<TokenType> types;
    types.push_back(TokenType::DAN);
    types.push_back(TokenType::ATAU);
    types.push_back(TokenType::TIDAK);
...
std::unique_ptr<ParserTree> Parser::parseOperatorPerbandingan() {
    std::vector<TokenType> types;
    types.push_back(TokenType::SAMA);
    types.push_back(TokenType::TIDAK_SAMA);
    types.push_back(TokenType::LEBIH_BESAR);
    types.push_back(TokenType::LEBIH_KECIL);
    types.push_back(TokenType::LEBIH_BESAR_SAMA_DENGAN);
    types.push_back(TokenType::LEBIH_KECIL_SAMA_DENGAN);
...
std::unique_ptr<ParserTree> Parser::parseOperasiPerbandingan() {
    std::unique_ptr<ParserTree> operasiPerbandingan = std::make_unique<ParserRuleTree>(ParserRule::operasi_perbandingan);
    std::unique_ptr<ParserTree> left = this->parseOperasiPrePost();
    operasiPerbandingan->addChild(std::move(left));
    while (this->matchOr({TokenType::SAMA, TokenType::TIDAK_SAMA, TokenType::LEBIH_BESAR, TokenType::LEBIH_KECIL, TokenType::LEBIH_BESAR_SAMA_DENGAN, TokenType::LEBIH_KECIL_SAMA_DENGAN})) {
        std::unique_ptr<ParserTree> operatorPerbandingan = this->parseOperatorPerbandingan();
...
std::unique_ptr<ParserTree> Parser::parseOperatorPrePost() {
    std::vector<TokenType> types;
    types.push_back(TokenType::TAMBAH_SATU);
    types.push_back(TokenType::KURANG_SATU);
...
std::unique_ptr<ParserTree> Parser::parseOperasiPrePost() {
    std::unique_ptr<ParserTree> operasiPrePost = std::make_unique<ParserRuleTree>(ParserRule::operasi_pre_post);
...
std::unique_ptr<ParserTree> Parser::parseOperatorPenjumlahan() {
    std::vector<TokenType> types;
    types.push_back(TokenType::TAMBAH);
    types.push_back(TokenType::KURANG);
...
std::unique_ptr<ParserTree> Parser::parseOperasiPenjumlahan() {
    std::unique_ptr<ParserTree> operasiPenjumlahan = std::make_unique<ParserRuleTree>(ParserRule::operasi_penjumlahan);
    ...
    while (this->matchOr({TokenType::TAMBAH, TokenType::KURANG})) {
        std::unique_ptr<ParserTree> operatorPenjumlahan = this->parseOperatorPenjumlahan();
...
std::unique_ptr<ParserTree> Parser::parseOperatorPerkalian() {
    std::vector<TokenType> types;
    types.push_back(TokenType::KALI);
    types.push_back(TokenType::BAGI);
    types.push_back(TokenType::SISA_BAGI);
...
std::unique_ptr<ParserTree> Parser::parseOperasiPerkalian() {
    std::unique_ptr<ParserTree> operasiPerkalian = std::make_unique<ParserRuleTree>(ParserRule::operasi_perkalian);
    ...
    while (this->matchOr({TokenType::KALI, TokenType::BAGI, TokenType::SISA_BAGI})) {
        std::unique_ptr<ParserTree> operatorPerkalian = this->parseOperatorPerkalian();
...
std::unique_ptr<ParserTree> Parser::parseNilai() {
    ...
    if(this->matchOr(types)) {
        ...
        std::unique_ptr<ParserTree> ekspresi_nilai = std::make_unique<ParserRuleTree>(ParserRule::nilai);
        std::unique_ptr<ParserTree> kurungBuka = std::make_unique<ParserTokenTree>(this->currentToken);
        ekspresi_nilai->addChild(std::move(kurungBuka));
        eat(TokenType::KURUNG_BUKA);
        std::unique_ptr<ParserTree> operatorPenugasan = this->parseOperasiPenugasan();
        ekspresi_nilai->addChild(std::move(operatorPenugasan));
        std::unique_ptr<ParserTree> kurungTutup = std::make_unique<ParserTokenTree>(this->currentToken);
    ...
    throw std::runtime_error(this->errorInfo.inLine(
        ...
        this->currentToken.getCharIndex(), 
        this->currentToken.getValue(), 
        std::format("'{}' bukan {} yang valid.", toString(this->currentToken.getType()), toString(ParserRule::nilai))
    ));
}
