...
OperatorPenugasanPenjumlahanContext::OperatorPenugasanPenjumlahanContext(nstd::bisa_kosong<Token> simbolOp): simbolOp(std::move(simbolOp)) {}

OperatorPenugasanPenjumlahanContext OperatorPenugasanPenjumlahanContext::generate(const std::vector<std::unique_ptr<ParserTree>>& children) {
    ...
    if(nstd::contains(tokenTypes, type)) {
        ...
        return OperatorPenugasanPenjumlahanContext(std::move(simbolOpPenugasanPenjumlahan));
    }
    throw std::runtime_error("Operator penugasan penjumlahan tidak valid");
}

nstd::bisa_kosong<Token> OperatorPenugasanPenjumlahanContext::getSimbolOp() const {
