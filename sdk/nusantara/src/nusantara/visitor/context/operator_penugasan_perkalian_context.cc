...
OperatorPenugasanPerkalianContext::OperatorPenugasanPerkalianContext(nstd::bisa_kosong<Token> simbolOp): simbolOp(std::move(simbolOp)) {}

OperatorPenugasanPerkalianContext OperatorPenugasanPerkalianContext::generate(const std::vector<std::unique_ptr<ParserTree>>& children) {
    ...
    if(nstd::contains(tokenTypes, type)) {
        ...
        return OperatorPenugasanPerkalianContext(std::move(simbolOpPenugasanPerkalian));
    }
    throw std::runtime_error("Operator penugasan perkalian tidak valid");
}

nstd::bisa_kosong<Token> OperatorPenugasanPerkalianContext::getSimbolOp() const {
