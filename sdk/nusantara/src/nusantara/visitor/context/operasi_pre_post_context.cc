...
OperasiPrePostContext OperasiPrePostContext::generate(const std::vector<std::unique_ptr<ParserTree>>& children) {
  nstd::bisa_kosong<std::unique_ptr<Context>> operasiPenjumlahanContext;
  nstd::bisa_kosong<std::unique_ptr<Context>> operatorPrePostContext;
  int index = 0;
  bool isPre = false;
  for(const std::unique_ptr<ParserTree>& child : children) {
    ...
    if(rule == ParserRule::operasi_penjumlahan) {
      operasiPenjumlahanContext = std::make_unique<OperasiPenjumlahanContext>(OperasiPenjumlahanContext::generate(ptchild->getChildren()));
    }else if(rule == ParserRule::operator_pre_post) {
      if(index == 0) {
        isPre = true;
      }
      operatorPrePostContext = std::make_unique<OperatorPrePostContext>(OperatorPrePostContext::generate(ptchild->getChildren()));
    }
    index++;
  }
  return {
...
bool OperasiPrePostContext::getIsPre() const {
    return this->isPre;
}

nstd::konst<nstd::bisa_kosong<std::unique_ptr<Context>>>& OperasiPrePostContext::getOperasiPenjumlahanContext() const {
    return this->operasiPenjumlahanContext;
}

nstd::konst<nstd::bisa_kosong<std::unique_ptr<Context>>>& OperasiPrePostContext::getOperatorPrePostContext() const {
