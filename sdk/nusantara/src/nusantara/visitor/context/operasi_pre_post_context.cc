...
OperasiPrePostContext::OperasiPrePostContext(
  ...
  const bool &isPre)
    : operasiPenjumlahanContext(std::move(operasiPenjumlahanContext)),
      operatorPrePostContext(std::move(operatorPrePostContext)), isPre(isPre)
{}
  ...
  const std::vector<std::unique_ptr<ParserTree>> &children)
{
      ...
      if(rule == ParserRule::operasi_penjumlahan)
        {
          operasiPenjumlahanContext
            = std::make_unique<OperasiPenjumlahanContext>(
  ...
  return {std::move(operasiPenjumlahanContext),
          std::move(operatorPrePostContext), isPre};
}
