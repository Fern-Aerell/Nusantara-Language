...
OperasiLogikaContext::OperasiLogikaContext(
    nstd::bisa_kosong<nstd::daftar<std::unique_ptr<Context>>> kumpulanOperasiPerbandinganContext,
    nstd::bisa_kosong<nstd::daftar<std::unique_ptr<Context>>> kumpulanOperatorLogikaContext
):
...
OperasiLogikaContext OperasiLogikaContext::generate(const std::vector<std::unique_ptr<ParserTree>>& children) {
  nstd::bisa_kosong<nstd::daftar<std::unique_ptr<Context>>> kumpulanOperasiPerbandinganContext;
  nstd::bisa_kosong<nstd::daftar<std::unique_ptr<Context>>> kumpulanOperatorLogikaContext;
  for(const std::unique_ptr<ParserTree>& child : children) {
    ...
    if(rule == ParserRule::operasi_perbandingan) {
      if(nstd::isKosong(kumpulanOperasiPerbandinganContext)) {
        kumpulanOperasiPerbandinganContext = nstd::daftar<std::unique_ptr<Context>>();
      }
      std::unique_ptr<Context> context = std::make_unique<OperasiPerbandinganContext>(OperasiPerbandinganContext::generate(ptchild->getChildren()));
      ...
      if(nstd::isKosong(kumpulanOperatorLogikaContext)) {
        kumpulanOperatorLogikaContext = nstd::daftar<std::unique_ptr<Context>>();
      }
      std::unique_ptr<Context> context = std::make_unique<OperatorLogikaContext>(OperatorLogikaContext::generate(ptchild->getChildren()));
...
nstd::konst<nstd::bisa_kosong<nstd::daftar<std::unique_ptr<Context>>>>& OperasiLogikaContext::getKumpulanOperasiPerbandinganContext() const {
    return this->kumpulanOperasiPerbandinganContext;
}

nstd::konst<nstd::bisa_kosong<nstd::daftar<std::unique_ptr<Context>>>>& OperasiLogikaContext::getKumpulanOperatorLogikaContext() const {
