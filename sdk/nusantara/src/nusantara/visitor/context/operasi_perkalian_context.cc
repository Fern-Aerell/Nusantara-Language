...
OperasiPerkalianContext OperasiPerkalianContext::generate(const std::vector<std::unique_ptr<ParserTree>>& children) {
  nstd::bisa_kosong<nstd::daftar<std::unique_ptr<Context>>> kumpulanNilaiContext;
  nstd::bisa_kosong<nstd::daftar<std::unique_ptr<Context>>> kumpulanOperatorPerkalianContext;
  for(const std::unique_ptr<ParserTree>& child : children) {
    ...
    if(rule == ParserRule::nilai) {
      ...
      std::unique_ptr<Context> context = std::make_unique<NilaiContext>(NilaiContext::generate(ptchild->getChildren()));
      ...
      if(nstd::isKosong(kumpulanOperatorPerkalianContext)) {
        kumpulanOperatorPerkalianContext = nstd::daftar<std::unique_ptr<Context>>();
      }
      std::unique_ptr<Context> context = std::make_unique<OperatorPerkalianContext>(OperatorPerkalianContext::generate(ptchild->getChildren()));
...
nstd::konst<nstd::bisa_kosong<nstd::daftar<std::unique_ptr<Context>>>>& OperasiPerkalianContext::getKumpulanNilaiContext() const {
    return this->kumpulanNilaiContext;
}

nstd::konst<nstd::bisa_kosong<nstd::daftar<std::unique_ptr<Context>>>>& OperasiPerkalianContext::getKumpulanOperatorPerkalianContext() const {
