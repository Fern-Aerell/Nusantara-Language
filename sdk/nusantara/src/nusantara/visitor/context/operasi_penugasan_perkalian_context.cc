...
OperasiPenugasanPerkalianContext::OperasiPenugasanPerkalianContext(
    nstd::bisa_kosong<nstd::daftar<std::unique_ptr<Context>>> kumpulanOperasiLogikaContext,
    nstd::bisa_kosong<nstd::daftar<std::unique_ptr<Context>>> kumpulanOperatorPenugasanPerkalianContext
):
...
OperasiPenugasanPerkalianContext OperasiPenugasanPerkalianContext::generate(const std::vector<std::unique_ptr<ParserTree>>& children) {
  nstd::bisa_kosong<nstd::daftar<std::unique_ptr<Context>>> kumpulanOperasiLogikaContext;
  nstd::bisa_kosong<nstd::daftar<std::unique_ptr<Context>>> kumpulanOperatorPenugasanPerkalianContext;
  for(const std::unique_ptr<ParserTree>& child : children) {
    ...
    if(rule == ParserRule::operasi_logika) {
      ...
      std::unique_ptr<Context> context = std::make_unique<OperasiLogikaContext>(OperasiLogikaContext::generate(ptchild->getChildren()));
      ...
      if(nstd::isKosong(kumpulanOperatorPenugasanPerkalianContext)) {
        kumpulanOperatorPenugasanPerkalianContext = nstd::daftar<std::unique_ptr<Context>>();
      }
      std::unique_ptr<Context> context = std::make_unique<OperatorPenugasanPerkalianContext>(OperatorPenugasanPerkalianContext::generate(ptchild->getChildren()));
      kumpulanOperatorPenugasanPerkalianContext.value().push_back(std::move(context));
    }
  }
  return {
...
nstd::konst<nstd::bisa_kosong<nstd::daftar<std::unique_ptr<Context>>>>& OperasiPenugasanPerkalianContext::getKumpulanOperasiLogikaContext() const {
    return this->kumpulanOperasiLogikaContext;
}

nstd::konst<nstd::bisa_kosong<nstd::daftar<std::unique_ptr<Context>>>>& OperasiPenugasanPerkalianContext::getKumpulanOperatorPenugasanPerkalianContext() const {
