  ...
  for(const std::unique_ptr<ParserTree>& child : children) {
    ...
    if(rule == ParserRule::operasi_penugasan_penjumlahan) {
      if(nstd::isKosong(kumpulanOperasiPenugasanPenjumlahanContext)) {
        kumpulanOperasiPenugasanPenjumlahanContext = nstd::daftar<std::unique_ptr<Context>>();
      }
      std::unique_ptr<Context> context = std::make_unique<OperasiPenugasanPenjumlahanContext>(OperasiPenugasanPenjumlahanContext::generate(ptchild->getChildren()));
      kumpulanOperasiPenugasanPenjumlahanContext.value().push_back(std::move(context));
    }else if(rule == ParserRule::operator_penugasan) {
      if(nstd::isKosong(kumpulanOperatorPenugasanContext)) {
        kumpulanOperatorPenugasanContext = nstd::daftar<std::unique_ptr<Context>>();
      }
      std::unique_ptr<Context> context = std::make_unique<OperatorPenugasanContext>(OperatorPenugasanContext::generate(ptchild->getChildren()));
      kumpulanOperatorPenugasanContext.value().push_back(std::move(context));
    }
  }
  return {
...
nstd::konst<nstd::bisa_kosong<nstd::daftar<std::unique_ptr<Context>>>>& OperasiPenugasanContext::getKumpulanOperasiPenugasanPenjumlahanContext() const {
    return this->kumpulanOperasiPenugasanPenjumlahanContext;
}

nstd::konst<nstd::bisa_kosong<nstd::daftar<std::unique_ptr<Context>>>>& OperasiPenugasanContext::getKumpulanOperatorPenugasanContext() const {
