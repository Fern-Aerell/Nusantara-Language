...
nstd::dinamis
Interpreter::operasiAritmatika(nstd::konst<nstd::dinamis> &left,
...
nstd::dinamis
Interpreter::operasiPenjumlahan(nstd::konst<nstd::dinamis> &left,
                                nstd::konst<nstd::dinamis> &right)
{
...
nstd::dinamis
Interpreter::operasiPengurangan(nstd::konst<nstd::dinamis> &left,
                                nstd::konst<nstd::dinamis> &right)
{
  if(nstd::isBulat(left) && nstd::isBulat(right))
    {
...
nstd::dinamis
Interpreter::operasiSisaPembagian(nstd::konst<nstd::dinamis> &left,
                                  nstd::konst<nstd::dinamis> &right)
{
  if(nstd::isBulat(left) && nstd::isBulat(right))
    {
  ...
  if(nstd::isDesimal(left) && nstd::isDesimal(right))
    {
      throw std::runtime_error("Tidak dapat melakukan operasi sisa pembagian "
...
nstd::dinamis
Interpreter::operasiPerbandingan(nstd::konst<nstd::dinamis> &left,
                                 nstd::konst<nstd::kalimat> &simbolOp,
                                 nstd::konst<nstd::dinamis> &right)
{
  if(simbolOp == "==")
...
nstd::dinamis Interpreter::operasiLebihBesar(nstd::konst<nstd::dinamis> &left,
                                             nstd::konst<nstd::dinamis> &right)
{
  if(nstd::isBulat(left) && nstd::isBulat(right))
    {
...
nstd::dinamis Interpreter::operasiLebihKecil(nstd::konst<nstd::dinamis> &left,
                                             nstd::konst<nstd::dinamis> &right)
{
  if(nstd::isBulat(left) && nstd::isBulat(right))
    {
...
nstd::dinamis
Interpreter::operasiLebihBesarSamaDengan(nstd::konst<nstd::dinamis> &left,
                                         nstd::konst<nstd::dinamis> &right)
{
  if(nstd::isBulat(left) && nstd::isBulat(right))
    {
...
nstd::dinamis
Interpreter::operasiLebihKecilSamaDengan(nstd::konst<nstd::dinamis> &left,
                                         nstd::konst<nstd::dinamis> &right)
{
  if(nstd::isBulat(left) && nstd::isBulat(right))
    {
  ...
  if(!nstd::isKosong(context))
    {
      for(nstd::konst<std::unique_ptr<Context>> &child : context.value())
        {
          auto *childPtr
            = dynamic_cast<OperasiPenugasanContext *>(child.get());
          if(childPtr != nullptr)
            {
              ...
              if(nstd::isKosong(result))
                ...
                {
                  std::cout << std::format(
                    "Hasil : {}", nstd::ubahKeKalimat(nstd::asBulat(result)))
                            << "\n";
                }
              else if(nstd::isDesimal(result))
                {
                  std::cout << std::format(
                    "Hasil : {}", nstd::ubahKeKalimat(nstd::asDesimal(result)))
                            << "\n";
                }
              else if(nstd::isKarakter(result))
                {
                  std::cout
                    << std::format("Hasil : {}", nstd::ubahKeKalimat(
                ...
                {
                  std::cout
                    << std::format("Hasil : {}", nstd::asKalimat(result))
                    << "\n";
                }
...
nstd::dinamis
Interpreter::visitOperatorPenugasan(nstd::konst<OperatorPenugasanContext> &ctx)
{
  return Interpreter::fragmentVisitOperator(ctx.getSimbolOp(), "penugasan");
}

nstd::dinamis
Interpreter::visitOperasiPenugasan(nstd::konst<OperasiPenugasanContext> &ctx)
{
  ...
  if(leftPtr == nullptr)
    {
      throw std::runtime_error(
        "Gagal mengubah Context menjadi OperasiPenugasanPenjumlahanContext");
    }
  ...
  for(size_t index = 0; index < simbolOp.value().size(); ++index)
    {
      const auto &simbolOpPtr = dynamic_cast<OperatorPenugasanContext *>(
        simbolOp.value()[index].get());
      const auto &rightPtr
        = dynamic_cast<OperasiPenugasanPenjumlahanContext *>(
          nilai.value()[index + 1].get());
      if(simbolOpPtr == nullptr || rightPtr == nullptr)
        {
          throw std::runtime_error(
            "Gagal mengubah Context menjadi OperatorPenugasanContext atau "
  ...
  nstd::konst<OperasiPenugasanPenjumlahanContext> &ctx)
{
  ...
  if(leftPtr == nullptr)
    {
      throw std::runtime_error(
        "Gagal mengubah Context menjadi OperasiPenugasanPerkalianContext");
    }
  ...
  for(size_t index = 0; index < simbolOp.value().size(); ++index)
    {
      ...
      if(simbolOpPtr == nullptr || rightPtr == nullptr)
        {
          throw std::runtime_error("Gagal mengubah Context menjadi "
                                   "OperatorPenugasanPenjumlahanContext atau "
                                   "OperasiPenugasanPerkalianContext");
        }
      nstd::konst<nstd::kalimat> simbolOp = nstd::asKalimat(
  ...
  nstd::konst<OperasiPenugasanPerkalianContext> &ctx)
{
  ...
  for(size_t index = 0; index < simbolOp.value().size(); ++index)
    {
      const auto &simbolOpPtr
        = dynamic_cast<OperatorPenugasanPerkalianContext *>(
          simbolOp.value()[index].get());
      const auto &rightPtr
      ...
      if(simbolOpPtr == nullptr || rightPtr == nullptr)
        {
          throw std::runtime_error(
            "Gagal mengubah Context menjadi OperatorPenugasanPerkalianContext "
...
nstd::dinamis
Interpreter::visitOperatorLogika(nstd::konst<OperatorLogikaContext> &ctx)
{
  return Interpreter::fragmentVisitOperator(ctx.getSimbolOp(), "logika");
}

nstd::dinamis
Interpreter::visitOperasiLogika(nstd::konst<OperasiLogikaContext> &ctx)
{
  ...
  for(size_t index = 0; index < simbolOp.value().size(); ++index)
    {
      ...
      if(simbolOpPtr == nullptr || rightPtr == nullptr)
        {
          throw std::runtime_error(
            "Gagal mengubah Context menjadi OperatorPerbandinganContext atau "
  ...
  for(size_t index = 0; index < simbolOp.value().size(); ++index)
    {
      const auto &simbolOpPtr = dynamic_cast<OperatorPerbandinganContext *>(
        simbolOp.value()[index].get());
      const auto &rightPtr = dynamic_cast<OperasiPrePostContext *>(
        nilai.value()[index + 1].get());
      if(simbolOpPtr == nullptr || rightPtr == nullptr)
        {
          throw std::runtime_error(
...
nstd::dinamis
Interpreter::visitOperatorPrePost(nstd::konst<OperatorPrePostContext> &ctx)
{
  return Interpreter::fragmentVisitOperator(ctx.getSimbolOp(), "pre post");
}

nstd::dinamis
Interpreter::visitOperasiPrePost(nstd::konst<OperasiPrePostContext> &ctx)
{
  ...
  nstd::konst<OperasiPenjumlahanContext> &ctx)
{
  ...
  for(size_t index = 0; index < simbolOp.value().size(); ++index)
    {
      const auto &simbolOpPtr = dynamic_cast<OperatorPenjumlahanContext *>(
        simbolOp.value()[index].get());
      const auto &rightPtr = dynamic_cast<OperasiPerkalianContext *>(
        nilai.value()[index + 1].get());
      if(simbolOpPtr == nullptr || rightPtr == nullptr)
        {
          throw std::runtime_error(
            "Gagal mengubah Context menjadi OperatorPenjumlahanContext atau "
            "OperasiPerkalianContext");
        }
      nstd::konst<nstd::kalimat> simbolOp
        = nstd::asKalimat(this->visitOperatorPenjumlahan(*simbolOpPtr));
      nstd::konst<nstd::dinamis> right
        = this->visitOperasiPerkalian(*rightPtr);
...
nstd::dinamis
Interpreter::visitOperatorPerkalian(nstd::konst<OperatorPerkalianContext> &ctx)
{
  return Interpreter::fragmentVisitOperator(ctx.getSimbolOp(), "perkalian");
}

nstd::dinamis
Interpreter::visitOperasiPerkalian(nstd::konst<OperasiPerkalianContext> &ctx)
{
  ...
  for(size_t index = 0; index < simbolOp.value().size(); ++index)
    {
      const auto &simbolOpPtr = dynamic_cast<OperatorPerkalianContext *>(
        simbolOp.value()[index].get());
      const auto &rightPtr
      ...
      if(simbolOpPtr == nullptr || rightPtr == nullptr)
        {
          throw std::runtime_error(
            "Gagal mengubah Context menjadi OperatorPerkalianContext atau "
            "NilaiContext");
        }
      nstd::konst<nstd::kalimat> simbolOp
