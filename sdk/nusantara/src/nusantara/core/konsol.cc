...
namespace nstd {
    void Konsol::cetak(konst<bulat>& bulat) {
        std::cout << bulat << "\n";
    }

    void Konsol::cetak(konst<desimal>& desimal) {
        std::cout << desimal << "\n";
    }
    ...
    void Konsol::cetak(konst<kalimat>& kalimat) {
        std::cout << kalimat << "\n";
    }
    ...
    void Konsol::cetak(konst<dinamis>& dinamis) {
        if(dinamis->has_value()) {
            if(isBulat(dinamis)) {
                ...
                std::cout << std::format("Tipe {} tidak bisa untuk di cetak.", dinamis.value().type().name()) << "\n";
            }
        }else{
            std::cout << "kosong" << "\n";
        }
