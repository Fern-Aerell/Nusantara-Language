...
namespace nstd {
  ...
  benarsalah isTidakAda(const dinamis &dinamis) {
    if(!isKosong(dinamis)) { return !dinamis.value().has_value(); }
    return false;
  }

  benarsalah isBulat(const dinamis &dinamis) {
    if(!isKosong(dinamis)) { return dinamis.value().type() == typeid(bulat); }
    return false;
  }

  benarsalah isDesimal(const dinamis &dinamis) {
    if(!isKosong(dinamis)) { return dinamis.value().type() == typeid(desimal); }
    return false;
  }
  ...
  benarsalah isKalimat(const dinamis &dinamis) {
    if(!isKosong(dinamis)) { return dinamis.value().type() == typeid(kalimat); }
    return false;
  }
  ...
  bulat asBulat(const dinamis &dinamis) {
    if(!isKosong(dinamis)) {
      if(isBulat(dinamis)) { return std::any_cast<bulat>(dinamis.value()); }
    }
  ...
  desimal asDesimal(const dinamis &dinamis) {
    if(!isKosong(dinamis)) {
      if(isDesimal(dinamis)) { return std::any_cast<desimal>(dinamis.value()); }
    }
  ...
  kalimat asKalimat(const dinamis &dinamis) {
    if(!isKosong(dinamis)) {
      if(isKalimat(dinamis)) { return std::any_cast<kalimat>(dinamis.value()); }
    }
    ...
    if(dinamis->has_value()) {
      if(isBulat(dinamis)) {
        ...
        throw std::runtime_error(std::format(
            "Tipe {} tidak bisa untuk di ubah ke kalimat.\n",
