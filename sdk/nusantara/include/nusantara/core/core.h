...
namespace nstd
{
  ...
  // Bisa Kosong
  template <class Tipe> using bisa_kosong = std::optional<Tipe>;

  // Konst
  template <class Tipe> using konst = const Tipe;
  ...
  // Daftar
  template <class Value> using daftar = std::vector<Value>;

  // Peta
  template <class Key, class Value> using peta = std::map<Key, Value>;

  // Dinamis
  using dinamis = bisa_kosong<std::any>;

  template <class Tipe> benarsalah isKosong(const bisa_kosong<Tipe> &tipe)
  {
  ...
  template <class Key, class Value> benarsalah isPeta(const dinamis &dinamis)
  {
    if(!isKosong(dinamis))
      {
  ...
  template <class Value> daftar<Value> asDaftar(const dinamis &dinamis)
  {
    if(!isKosong(dinamis))
      {
  ...
  template <class Value> kalimat ubahKeKalimat(konst<daftar<Value>> &daftar)
  {
